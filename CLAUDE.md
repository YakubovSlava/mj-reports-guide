...
- [ ] Внешние JS/CSS через CDN допустимы — платформа имеет доступ к интернету
...
> Скрипты запускаются на сервере платформы — сетевые запросы технически возможны,
> но данные лучше передавать через CSV-файл источника данных навыка, а не тянуть
> их в рантайме скрипта (медленнее, хрупче, не кэшируется).
...
**Для интерактивных графиков** — Plotly (два варианта):
```python
import plotly.graph_objects as go
fig = go.Figure(...)

# Вариант А — через CDN (быстрее загрузка страницы, требует интернета):
print(fig.to_html(include_plotlyjs='cdn', full_html=False))

# Вариант Б — встроенный JS (~3 МБ, работает без интернета):
print(fig.to_html(include_plotlyjs=True, full_html=False))
```

**Для статичных графиков** — matplotlib + base64:
```python
