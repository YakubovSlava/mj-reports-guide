...
- [ ] Нет импортов за пределами стандартной библиотеки + `pandas`, `openpyxl`
...
**Пользователь хочет установить библиотеку:**
> Скрипты запускаются в уже настроенном окружении платформы, установить новые пакеты
> нельзя. Доступны: стандартная библиотека Python, `pandas`, `openpyxl`.
> Давай перепишем без этой библиотеки — я помогу.
...
- Таймаут: 60 секунд
- Доступно: stdlib + `pandas`, `openpyxl`
