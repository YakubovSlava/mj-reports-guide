...
- Сгенерируй синтетику: `python generate_synthetic.py <format> --out data/test_<format>.csv`
...
**Это обязательный первый шаг. Нельзя пропустить.**

Выполни сам (если есть доступ к терминалу):
```bash
python generate_synthetic.py <format> --out data/test_<format>.csv
```

Если терминала нет — попроси пользователя выполнить команду и дождись подтверждения.
